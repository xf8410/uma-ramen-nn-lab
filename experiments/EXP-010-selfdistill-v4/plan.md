...
- **禁静默回退**：`RAMEN_NN_MODEL` 缺失/加载失败即 panic——否则教师悄悄退回手写
  相当于白跑 160k 采集，必须 fail-fast

## 工作流（exp-010.yml，结构 = exp-009.yml 改 3 处）

1. **collect 前置：下载 009b 冠军模型 artifact**（run 33744258184 的 EXP-009b-nn009-0831）
2. **collect**：80 分片 × CHUNK=2000（同 009 采集量），patch 链 006c→d→e→fix→**010**
   - 环境：`RAMEN_NN_MODEL=models/009-champion/model.onnx`
   - patch 后可执行文件必须链上 onnx feature（cargo build --features onnx）
3. **assemble/convert/labels/train**：全部同 009（3 种子 ep400 + 12k 对照臂）
4. **裁决 EXP-010b**：四基线（默认 65438.2 / 冠军 65554.2 / gen3 65669.2 / gen4 66734.8）
